...
// Waymo-style reveal: the whole headline appears on a dark highlight with a
// blue caret (line + dot) at its end; about 1 second later both fade away.
export function TypedHeadline({ text, className }: { text: string; className?: string }) {
  ...
  return <h1 className={className}>
    <span className={`[box-decoration-break:clone] [-webkit-box-decoration-break:clone] transition-colors duration-700 ${done ? "bg-transparent" : "bg-[#131D33]/85"}`}>{text}</span>
