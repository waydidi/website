...
export function BlogCover({ post, size = "large", chips = true }: { post: BlogPost; size?: "large" | "thumb"; chips?: boolean }) {
  ...
  return <div className={`relative overflow-hidden bg-gradient-to-br ${TONES[post.cover.tone]} ${large ? "aspect-[16/8.6] rounded-[20px]" : "aspect-[4/3] rounded-[14px]"}`}>
    ...
    <p className={`relative whitespace-pre-line font-extrabold leading-[1.02] tracking-[-.02em] text-white drop-shadow-sm ${large ? "max-w-[58%] p-5 text-[clamp(20px,5.6vw,38px)]" : "max-w-[92%] p-2.5 text-[13px]"}`}>{post.cover.headline}</p>
    {large && chips && <div className="absolute bottom-3 left-3 flex flex-wrap gap-2">
      {post.categories.map((c) => <span key={c} className="rounded-full bg-white px-3 py-1 text-[13px] italic text-[#E07400] shadow-sm">{BLOG_CATEGORIES[c]}</span>)}
    </div>}
  </div>;
}
