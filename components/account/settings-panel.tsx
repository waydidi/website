...
export function SettingsPanel({ marketingOptIn, deviceCount }: { marketingOptIn: boolean; deviceCount: number }) {
  ...
  async function deleteAccount() {
    ...
    const response = await fetch("/api/account", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ confirm: confirmText }) });
    if (response.ok) return window.location.assign("/");
    setBusy(""); setError("Could not delete your account. Please try again.");
  }
  ...
  return <div className="grid gap-5">
    ...
    <section className={card} aria-labelledby="privacy">
      ...
      <p className="mt-1 text-sm text-slate-600">Download a copy of your profile and bookings, or delete your account. See the <Link href="/privacy" className="font-bold text-[#C96100] hover:underline">privacy notice</Link>.</p>
      ...
      <div className="mt-7 rounded-2xl border border-red-100 bg-red-50/60 p-5">
        <h3 className="font-black text-red-800">Delete account</h3>
        <p className="mt-1 text-sm text-red-900/80">This removes your account, profile and sign-in sessions. Booking records are kept for accounting and legal reasons, and you can still manage an existing booking with its reference and surname.</p>
        <label className="mt-4 block text-sm font-bold text-red-900">Type DELETE to confirm<input value={confirmText} onChange={(e) => setConfirmText(e.target.value)} className="mt-2 w-full rounded-xl border border-red-200 bg-white px-4 py-2.5 outline-none focus:border-red-400 sm:w-60" /></label>
        <button onClick={deleteAccount} disabled={confirmText !== "DELETE" || !!busy} className="mt-4 inline-flex items-center gap-2 rounded-full bg-red-600 px-5 py-2.5 font-black text-white disabled:opacity-50"><Trash2 size={18} />{busy === "delete" ? "Deleting…" : "Delete my account"}</button>
      </div>
