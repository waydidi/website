...
const statusStyle: Record<string, string> = {
  ...
  going_to_standby: "bg-blue-100 text-blue-800",
  standby: "bg-amber-100 text-amber-800",
  passenger_verified: "bg-cyan-100 text-cyan-800",
      ...
      if (result.driverUrl)
        setLatestLink({
          reference:
            ...
            data?.assignments.find((item) => item.id === payload.assignmentId)
              ?.bookingReference ??
            "Booking",
          url: result.driverUrl,
        });
      await load();
      return true;
    } catch (cause) {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            {assignment ? (
                              <span
                                className={`rounded-full px-3 py-1 text-xs font-black ${statusStyle[assignment.currentStatus] ?? statusStyle.assigned}`}
                              >
                                {statusLabel[assignment.currentStatus] ??
            ...
            <aside className="h-fit rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-5">
              {selectedBooking ? (
                <>
                  ...
                  {selectedAssignment && (
                    <div className="mt-4 flex items-center justify-between rounded-2xl border border-orange-200 bg-orange-50 p-4">
                      ...
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-black ${statusStyle[selectedAssignment.currentStatus] ?? statusStyle.assigned}`}
                      >
                        {statusLabel[selectedAssignment.currentStatus]}
                      </span>
                    </div>
                  )}
                  {selectedLocation && (
                    <a href={`https://www.google.com/maps?q=${selectedLocation.latitude},${selectedLocation.longitude}`} target="_blank" rel="noreferrer" className="mt-3 flex items-center justify-between rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
                      <span><strong className="block">Latest live location</strong><span className="text-blue-700">{new Date(selectedLocation.serverTimestamp).toLocaleString("en-GB", { timeZone: "Asia/Bangkok" })} · ±{selectedLocation.accuracyMetres}m</span></span>
                      <span className="flex items-center gap-1 font-black">Map <ExternalLink size={15}/></span>
                    </a>
                  )}
                  {selectedException && (
                    <div className="mt-3 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-amber-950">
                      <div className="flex items-center justify-between gap-3"><strong>{selectedException.exceptionType === "abnormal_stop" ? "Unexplained prolonged stop" : "Route deviation"}</strong><span className="rounded-full bg-amber-200 px-2.5 py-1 text-xs font-black">Review</span></div>
                      <p className="mt-2 text-sm">{selectedException.exceptionType === "abnormal_stop" ? `${Math.round((selectedException.stopDurationSeconds ?? 0) / 60)} minutes stationary · ${(selectedException.distanceMetres / 1000).toFixed(1)} km remaining` : `${(selectedException.distanceMetres / 1000).toFixed(1)} km from expected route`}</p>
                      <p className="mt-1 text-xs text-amber-800">Started {new Date(selectedException.startedAt).toLocaleTimeString("en-GB", { timeZone: "Asia/Bangkok" })} · Last location {new Date(selectedException.lastSeenAt).toLocaleTimeString("en-GB", { timeZone: "Asia/Bangkok" })}</p>
                    </div>
                  )}
                  <div className="mt-6">
                    <h3 className="font-black">Driver confirmations</h3>
                    <div className="mt-4 space-y-4">
                      {selectedEvents.map((event) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span
                              className={`rounded-full px-2.5 py-1 text-xs font-bold ${event.verificationStatus === "verified" ? "bg-emerald-100 text-emerald-700" : event.verificationStatus === "rejected" ? "bg-red-100 text-red-700" : event.verificationStatus === "not_required" ? "bg-slate-100 text-slate-600" : "bg-amber-100 text-amber-800"}`}
                            >
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {event.evidenceKey && (
                              <a
                                ...
                                target="_blank"
                                className="flex items-center gap-1.5 rounded-full border border-slate-200 px-3 py-2 text-xs font-bold"
                              >
                                <Camera size={15} />
                                View picture
                              </a>
                          ...
                          {event.expectedDistanceMetres != null && (
                            <p
                              className={`mt-3 text-xs font-bold ${event.expectedDistanceMetres > 500 ? "text-red-700" : "text-slate-500"}`}
                            >
                              {event.expectedDistanceMetres.toLocaleString()} m
                              from scheduled location
                            </p>
                          )}
                          {event.verificationStatus === "pending_review" && (
                            <div className="mt-4 grid grid-cols-2 gap-2">
                              <button
                                ...
                                className="flex h-10 items-center justify-center gap-1 rounded-full bg-emerald-600 text-xs font-bold text-white"
                              >
                                <Check size={15} />
                                Verify
                              </button>
                              <button
                                ...
                                className="flex h-10 items-center justify-center gap-1 rounded-full bg-red-50 text-xs font-bold text-red-700"
                              >
                                <XCircle size={15} />
                                Reject
                              </button>
                      ...
                      {selectedEvents.length === 0 && (
                        <div className="rounded-2xl border border-dashed border-slate-300 p-7 text-center text-sm text-slate-500">
                          <Truck className="mx-auto mb-3 text-slate-300" />
                ...
                <div className="py-16 text-center text-slate-500">
                  Choose a journey to see its driver activity.
                </div>
              )}
            </aside>
          )}
