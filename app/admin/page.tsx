  ...
  return <main className="min-h-screen bg-[#F6F7F9] px-4 py-6 text-[#1f1726] sm:px-8">
    <div className="mx-auto grid max-w-[1200px] gap-6">
      ...
      <nav aria-label="Quick links" className="flex flex-wrap gap-2 pb-6">
        {[["Bookings", "/admin/bookings"], ["Driver dispatch", "/admin/dispatch"], ["Booking operations", "/admin/operations"], ["Pricing", "/admin/pricing"], ["Promotions", "/admin/promotions"], ["Member gifts", "/admin/gifts"], ["Blog", "/admin/blog"]].map(([l, h]) => <Link key={h} href={h} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-[13px] font-bold hover:border-[#FF8A05]">{l}</Link>)}
      </nav>
    </div>
  </main>;
}
