...
function BookingPanel({ booking, driver, alerts, notifications, busy, onSave }: { booking: Booking; driver?: Driver; alerts: OperationsAlert[]; notifications: Notification[]; busy: string; onSave: (updates: Record<string, unknown>) => Promise<boolean> }) {
  ...
  return <>
    ...
    <div className="space-y-5 px-6 pb-8">
      ...
      <section className="rounded-2xl border border-slate-200 p-4"><h3 className="font-black">Driver assignment</h3>{booking.assignment ? <div className="mt-3 flex items-center justify-between rounded-xl bg-orange-50 p-3"><div><p className="font-black">{driver?.fullName ?? "Assigned driver"}</p><p className="text-sm text-slate-500">{statusNames[booking.assignment.currentStatus] ?? booking.assignment.currentStatus}</p></div><Truck className="text-[#D96F00]" /></div> : <a href={`/admin/journeys/${encodeURIComponent(booking.reference)}`} className="mt-3 inline-flex rounded-full bg-[#FF8A05] px-5 py-2 text-sm font-bold text-white">Assign in Driver information →</a>}</section>
