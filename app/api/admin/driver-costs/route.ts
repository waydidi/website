...
export async function POST(request:Request){
  ...
  if(input.action==="assign_driver"||input.action==="rotate_link"){
    ...
    if(input.action==="assign_driver"){
      ...
      for(const assignment of driverAssignments.filter(row=>row.bookingReference!==bookingReference&&row.currentStatus!=="completed"&&row.currentStatus!=="no_show")){const assigned=bookingMap.get(assignment.bookingReference);if(!assigned||assigned.status!=="confirmed")continue;const occupied=bookingWindow(assigned);if(rangesOverlap(candidate.startsAt,candidate.endsAt,occupied.startsAt,occupied.endsAt))conflicts.push(`Overlaps ${assigned.reference} at ${assigned.pickupTime}`);}
