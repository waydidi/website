...
export async function GET(_request: Request, context: { params: Promise<{ driverId: string; kind: string }> }) {
  ...
  if (!key || !env.BUCKET) return NextResponse.json({ error: "Image not found." }, { status: 404 });
  const object = await env.BUCKET.get(key);
  if (!object) return NextResponse.json({ error: "Image not found." }, { status: 404 });
  return new Response(object.body, { headers: { "Content-Type": mime ?? object.httpMetadata?.contentType ?? "image/jpeg", "Cache-Control": "private, no-store", "Content-Disposition": `inline; filename="driver-${kind}"`, "X-Content-Type-Options": "nosniff" } });
}
