...
export async function createConfirmationPdf(booking: Confirmation, extras?: BookingExtras) {
  ...
  const logoH = 56;
  page.drawImage(logo, { x: 33, y: 770, width: (logo.width / logo.height) * logoH, height: logoH });
  page.drawText("Your ride is booked", { x: 46, y: 743.5, size: 29, font: bold, color: rgb(1, 1, 1) });
  // Right column: label with the reference under it, bottom-aligned with the headline.
  page.drawText("Booking reference", { x: 549 - bold.widthOfTextAtSize("Booking reference", 14), y: 759.5, size: 14, font: bold, color: rgb(1, 1, 1) });
