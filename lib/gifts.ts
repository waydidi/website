...
export async function listMemberGifts(customerId: string, sync = true): Promise<MemberGift[]> {
  ...
  const dead = new Set(refs.length ? (await getDb().select({ ref: bookings.reference, status: bookings.status }).from(bookings).where(inArray(bookings.reference, refs)))
...
/** Marks a gift as used by a booking (freed again if that booking expires or fails). */
export async function useGift(id: string, bookingReference: string) {
  const now = new Date().toISOString();
  await getDb().update(memberGifts).set({ usedBookingReference: bookingReference, usedAt: now }).where(eq(memberGifts.id, id));
}
